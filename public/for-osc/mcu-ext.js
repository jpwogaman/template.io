...
function mcuToOsc(host, port, address, args) {
    if (host !== 'midi' ?? port !== MIDI_DEVICE_NAME ?? port !== MIDI_DEVICE_NAME_EXT) return //NOSONAR
    ...
    if (address === '/note') {
        ...
        if (value.includes('f0 00 00 66 14 12')) {
            ...
            if (port === MIDI_DEVICE_NAME) {
                ...
                receive('midi', MIDI_DEVICE_NAME_EXT, '/led_signal', channel, mode & 1 ? 1 : 0)
            }
        }
    }
...
if (!midi) {
    ...
    if (!midi.includes(MIDI_DEVICE_NAME_EXT)) {
        console.warn(`(WARNING, MCU) "midi" device "${MIDI_DEVICE_NAME_EXT}" not declared`)
    }
}
