...
export const RangeRows: FC<RangeRowProps> = ({ id }) => {
  ...
  const artIndex: number = parseInt(id) - 1
  // first id would be "01"... need to figure out "indexOf" here
  ...
  const rangeTdEmpty = `bg-zinc-400 
        dark:bg-zinc-600
        border-2 
        border-zinc-400 
        dark:border-zinc-600
  ...
  return (
    <Fragment>
      {RangeList?.map((range) => (
        <tr
          key={`ArtRange_${id}_${range.id}`}
          id={`ArtRange_${id}_${range.id}`}
          className={`${rangeTr}`}>
          <td
            colSpan={2}
            className={`${rangeTdEmpty}`}></td>
          ...
          <td className={`${rangeTd} text-center`}>
            <IconBtnToggle
              ...
              onToggleB={() => removeRange(id, range.id)}></IconBtnToggle>
          </td>
          <td className={`${rangeTd} border-r-transparent text-center`}>
            <input
              type='checkbox'
              ...
              id={`FullRangeWhiteKeysCheck_trk_${parseInt(selectedTrack.id)}_${
                range.id
              }`}></input>
          </td>
          <td className={`${rangeTd} text-left text-xs`}>
            <p>white-keys-only</p>
          </td>
